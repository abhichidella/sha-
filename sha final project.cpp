...
vector<uint8_t> sha256_pad(const string &input) {
...
string sha256(const string &input) {
    vector<uint8_t> msg = sha256_pad(input);
    ...
    stringstream ss;
    for (int i = 0; i < 8; i++) ss << hex << setw(8) << setfill('0') << H[i];
    return ss.str();
}

int main() {
    cout << "Enter or paste the full text (Book of Mark):\n";
    string input, line;
